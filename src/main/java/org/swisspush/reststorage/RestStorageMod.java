...
public class RestStorageMod extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) {
        ...
        PathProcessingStrategyFinder pathProcessingStrategyFinder = new PathProcessingStrategyFinder(modConfig.getPathProcessingStrategy());

        Handler<HttpServerRequest> handler = new RestStorageHandler(vertx, log, storage, modConfig.getPrefix(),
                modConfig.getEditorConfig(), modConfig.isConfirmCollectionDelete(), modConfig.isRejectStorageWriteOnLowMemory(), pathProcessingStrategyFinder);
